...
func (r *Raft) applyCommand(req *pb.ApplyCommandRequest) (*pb.ApplyCommandResponse, error) {
	...
	logEntry := pb.Entry{Id: lastLogId + 1, Term: r.currentTerm, Data: req.GetData()}
	r.appendLogs([]*pb.Entry{&logEntry})

	// TODO: (B.1)* - return the new log entry
	return &pb.ApplyCommandResponse{Entry: &logEntry}, nil
}
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	if req.GetTerm() < r.currentTerm {
		...
		return &pb.RequestVoteResponse{Term: r.currentTerm, VoteGranted: false}, errors.New("reject request vote since current term is older")
	}
...
func (r *Raft) broadcastAppendEntries(ctx context.Context, appendEntriesResultCh chan *appendEntriesResult) {
	...
	for peerId, peer := range r.peers {
		...
		allNewEntry := r.getLogs(r.nextIndex[peerId])
		req := &pb.AppendEntriesRequest{
			Term:           r.currentTerm,
			LeaderId:       r.id,
		...
		r.logger.Debug("send append entries", zap.Uint32("peer", peerId), zap.Any("request", req), zap.Int("entries", len(allNewEntry)))
...
func (r *Raft) handleAppendEntriesResult(result *appendEntriesResult) {
	...
	if !result.GetSuccess() {
		...
		nextIndex, matchIndex := r.nextIndex[result.peerId]-1, r.matchIndex[result.peerId]
	...
	for i := len(logs) - 1; i >= 0; i-- {
		...
		if logs[i].GetTerm() == r.currentTerm {
			...
			for pid := range r.peers {
				if r.matchIndex[pid] >= logs[i].GetId() {
					replicas++
				}
			}
